...
#           {'device_type': 'None', 'bt_mac': '00087B18E51B', 'name': 'TAG 2', 'account': 'Passive', 'uuid': 'empty', 'beacon_type': 'None', 'proximity': 'None', 'beacon_gateway' : 'None', 'user_image': '/images/bed.jpeg', 'device_loggedin' : '1', 'base_location': 'None'},
           {'device_type': 'handset', 'bt_mac': '000413B50038', 'name': 'M90 Snom Medical', 'account': '7008', 'uuid': 'empty', 'beacon_type': 'None', 'proximity': 'None', 'beacon_gateway' : 'None', 'user_image': '/images/Heidi_MacMoran_small.jpg', 'device_loggedin' : '1', 'base_location': 'None'}
#           {'device_type': 'None', 'bt_mac': '00087B177E9F', 'name': 'M80 Alarm Message', 'account': '7003', 'uuid': 'empty', 'beacon_type': 'None', 'proximity': 'None', 'beacon_gateway' : '0815', 'user_image': '/images/Michelle_Simmons_small.jpg', 'device_loggedin' : '1', 'base_location': 'None'},
#           {'device_type': 'None', 'bt_mac': '000413B40044', 'name': 'M80 Alarm Call', 'account': '7004', 'uuid': 'empty', 'beacon_type': 'None', 'proximity': 'None', 'beacon_gateway' : 'None','user_image': '/images/Steve_Fuller_small.jpg', 'device_loggedin' : '1', 'base_location': 'None'},
...
class MSeriesAlarm:
    ...
    def update_beacon(self, messageuui, senderaddress, beacon_gateway='None'):
        print(messageuui)

        if messageuui.split(';',1)[0] == '!BT':
            _, bt_mac, _, beacon_type, uuid, _, proximity, _= messageuui.split(';')
        else:
        ...
        if not matched_bt_mac:
            # add a new bt_mac
            self.devices.append({'device_type': 'beacon', 'bt_mac': bt_mac, 'name': 'unknown Beacon', 'account': 'unknown', 'uuid': uuid, 'beacon_type': beacon_type, 'proximity': proximity, 'beacon_gateway' : 'None', 'user_image': '/images/depp.jpg', 'device_loggedin' : '1', 'base_location': 'None', 'last_beacon': 'beacon ping'})
            
            print('added: ', bt_mac, ' ', uuid)
...
#            print('old   :', matched_bt_mac)
    ...
    def update_login(self, device_type, login_name, login_address, login, base_location):
        ...
        if not matched_address:
            # add a new device
            self.devices.append({'device_type': device_type, 'bt_mac': 'None', 'name': login_name, 'account': login_address, 'uuid': '', 'beacon_type': 'None', 'proximity': 'None', 'beacon_gateway' : 'None', 'user_image': '/images/depp.jpg', 'device_loggedin' : login, 'base_location': base_location, 'last_beacon': 'None'})
    ...
    def add_senderdata(self, xml_root):
        ...
        names = xml_root.xpath("//senderdata/name")
        for name in names:
            # XPATH uses index 1..
            element_idx = names.index(name) + 1
            print(name)
            ...
            self.update_login('handset', name_txt, address_txt, "1", base_location)
    ...
    def send_xml(self, xml_data):
        ...
        # print(str(ET.tostring(xml_with_header, pretty_print=True, encoding="unicode")))

        s.sendto(xml_with_header, m900_connection)
    ...
    def send_to_location_viewer(self):
        r = requests.post('http://localhost:8080/en_US/location', json=self.devices)
        return True
        ...
        if request_type:
            ...
            if request_type == 'systeminfo':
                ...
                if senderdata:
                    ...
                    print('keep_alive: Respond with MS confirm response to FP:')
                    # send ready to operate response
                    self.response_keepalive(self.externalid, status, statusinfo)
                else:
                    print('systeminfo: Respond with MS confirm response to FP:')
...
#     </request>
            ...
            if request_type == 'job':
                print('Job: ', data)
                dd = alarm_profile_root.xpath(self.msg_xpath_map['JOB_REQUEST_JOBDATA_MESSAGEUUID_XPATH'])
                if dd:
                    print('Beacon')
...
#
                    ...
                    print('FATAL, we expected beacondata', data)
...
m900_connection  = ('192.168.188.22', 1300)
