...
def send_locations():
    if msgDb:
        ...
        for btmac in btmacs:
            ...
            # select the BEST
            best = get_best_m9b(selected_items)
            mqttc.publish_beacon_full_state_ha(best['bt_mac'], "Mxx", best['account'],
                                               best['proximity'], best['rssi'],
                                               best['beacon_gateway_IPEI'],
                                               best['beacon_gateway_name'],
                                               gateway_list
                                               )
            # mqtt_room state
            mqttc.publish_beacon_room_state_ha(best['bt_mac'], best['account'],
                                               best['proximity'], best['rssi'],
                                               best['beacon_gateway_IPEI'], best['beacon_gateway_name'])
